...
program::program(std::vector<long> &source_code)
{
    intcode = source_code;
}

void program::run(std::vector<int> &save)
{
    for (; iterator < intcode.size();)
    {
        ...
        case 3: //input
            long input;
            std::cin >> input;
            intcode[find(intcode, iterator, 1)] = input;
            iterator += 2;
            break;

        case 4: //output
            //save.push_back(intcode[find(intcode, iterator, 1)]);
            std::cout << intcode[find(intcode, iterator, 1)] << '\n';
...
long program::find(std::vector<long> const &_inputVector, long _iterator, long _offset)
{
    ...
    {
        std::cerr << "mode unknown" << std::endl; // does not stop it
        return -1;
    }
}
